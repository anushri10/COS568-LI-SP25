...
# #!/usr/bin/env bash
# set -e
# echo "Hybrid‑PGM sweep…"

# BENCH=build/benchmark
# [ -f "$BENCH" ] || { echo "benchmark binary missing"; exit 1; }

# mkdir -p results_hybrid
# FLUSH_START=(100000 500000 1000000)      # starting thresholds

# run_case () {             #  $1 = dataset   $2 = ops_file  $3 = tag
#   local DS=$1 OPS=$2 TAG=$3
#   for TH in "${FLUSH_START[@]}"; do
#     echo "[$TAG] $DS  TH=$TH"
#     "$BENCH" "./data/$DS" "./data/$OPS" \
#        --through --csv \
#        --only HybridPGM --value "$TH" -r 3 \
#        >  /dev/null
#     # mv latest CSV into a separate dir (easy filtering later)
#     mv results/*HybridPGM*.csv results_hybrid/ 2>/dev/null || true
#   done
# }

# for DS in fb_100M_public_uint64 books_100M_public_uint64 osmc_100M_public_uint64
# do
#   run_case "$DS" "${DS}_ops_2M_0.000000rq_0.500000nl_0.100000i_0m_mix" "mix‑90Lkp"
#   run_case "$DS" "${DS}_ops_2M_0.000000rq_0.500000nl_0.900000i_0m_mix" "mix‑10Lkp"
# done

# echo "Hybrid sweep complete – CSV files in results_hybrid/"
